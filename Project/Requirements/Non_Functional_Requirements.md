# Non Functional Requirements

## Member 1 Contributions — Mahra Alhammadi (g00098713)

| NFR ID | Category | Non-Functional Requirement | Contributor |
| --- | --- | --- | --- |
| NFR-01 | Performance | Found item search results shall be displayed within 2 seconds for a database of up to 5,000 found item records. | Mahra Alhammadi (g00098713) |
| NFR-02 | Usability | A first-time user with no training shall be able to complete a lost item report in under 3 minutes, measured with at least 5 test users. | Mahra Alhammadi (g00098713) |
| NFR-03 | Portability | The system shall work correctly on 4 target browsers (Chrome, Safari, Edge and Firefox, latest two versions of each) at screen widths from 360 px (phone) to 1920 px (desktop). | Mahra Alhammadi (g00098713) |
| NFR-04 | Reliability | The system shall have an availability of at least 99% during the academic semester, excluding scheduled maintenance announced at least 24 hours in advance. | Mahra Alhammadi (g00098713) |
| NFR-05 | Robustness | If a lost item report or claim submission is interrupted, for example by a lost connection, the system shall not store a partial or duplicate record. In 50 simulated interrupted submissions, 0 such records shall be created. | Mahra Alhammadi (g00098713) |
## Member 2 Contributions — Hamda Raed (g00100146)

| NFR ID | Category | Non-Functional Requirement | Contributor |
| --- | --- | --- | --- |
| NFR-06 | Performance | The matching algorithm shall return candidate matches within 3 seconds for a database of up to 10,000 active reports. | Hamda Raed (g00100146) |
| NFR-07 | Reliability | The system shall deliver match notification emails within 5 minutes of a match being generated, with successful delivery confirmed for at least 98% of notifications sent. | Hamda Raed (g00100146) |
| NFR-08 | Scalability | The system shall support at least 500 concurrent found-item report submissions without response time degrading more than 2x baseline. | Hamda Raed (g00100146) |
| NFR-09 | Size | Each photo attached to a found-item report shall not exceed 5MB, with a maximum of 3 photos per report. | Hamda Raed (g00100146) |
| NFR-10 | Maintainability | The match-scoring logic shall be implemented as a separate module so matching criteria can be updated without modifying the reporting or notification modules. | Hamda Raed (g00100146) |


## Member 3 Contributions — Hamda Albahri (g00100284)

| NFR ID | Category | Non-Functional Requirement | Contributor |
| --- | --- | --- | --- |
| NFR-11 | Security | Access to the admin dashboard shall be restricted to authenticated campus security accounts; attempts by regular student or staff accounts to access admin functions shall be denied. | Hamda Albahri (g00100284) |
| NFR-12 | Security | A campus security account shall be automatically logged out after 15 minutes of inactivity and shall require authentication again before accessing the admin dashboard. | Hamda Albahri (g00100284) |
| NFR-13 | Privacy | Private identifying details submitted for claim verification shall only be visible to authorized campus security accounts and shall never be displayed in public lost or found item listings. | Hamda Albahri (g00100284) |
| NFR-14 | Auditability | Every claim approval or rejection shall be recorded with the responsible campus security account, action performed, and date and time, and the audit record shall remain available for the duration of the academic semester. | Hamda Albahri (g00100284) |
| NFR-15 | Security | After 5 consecutive failed login attempts on a campus security account within 10 minutes, the system shall temporarily block further login attempts to that account for 15 minutes. | Hamda Albahri (g00100284) |

## Team Consolidated

_To be added after the oral._
